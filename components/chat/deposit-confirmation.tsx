...
export type DepositConfirmationProps = {
  id: string;
  amount: string;
  chainId: number;
  chainName: string;
...
export function DepositConfirmation({
  ...
  const tokenLabel = erc20 ? truncateAddress(erc20) : nativeSymbol;
  ...
  return (
    <div className="my-4 max-w-2xl">
      <div className="mb-3">
        <h3 className="text-base font-medium">Confirm deposit</h3>
      </div>
      ...
      <div className="rounded-xl bg-sidebar dark:bg-card p-4 mb-3 space-y-3">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm text-foreground">({chainName})</span>
          </div>
        </div>
